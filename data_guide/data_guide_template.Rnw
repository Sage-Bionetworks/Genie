...
Each sample in GENIE is associated to a SEQ\_ASSAY\_ID, which is the institutional assay identifier for genomic testing platform.  Due to several sites that actually have many different SEQ\_ASSAY\_ID's that use same workflow, we have introduced SEQ\_PIPELINE\_ID, which is present in the assay information file.  For many of the sites the SEQ\_PIPELINE\_ID will be same as the SEQ\_ASSAY\_ID.
