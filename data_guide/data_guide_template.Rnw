...
@
\end{center}
\caption{Distribution of library selection, library strategy, platform, and specimen tumor cellularity across pipelines}
\end{figure}
...
info_tab = xtable(specimen_per_panel,
                  caption = 'Preservation Techniques per Panel')
