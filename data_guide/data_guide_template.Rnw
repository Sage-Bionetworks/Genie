...
Each sample in GENIE is associated to a SEQ_ASSAY_ID, which is the institutional assay identifier for genomic testing platform.  Due to several sites that actually have many different SEQ_ASSAY_ID's that use same workflow, we have introduced SEQ_PIPELINE_ID, which is present in the assay information file.  For many of the sites the SEQ_PIPELINE_ID will be same as the SEQ_ASSAY_ID.
