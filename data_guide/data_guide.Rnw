...
#genomic_profiledf$genomic_profile = gsub("\\%", "\\%",
#                                         genomic_profiledf$genomic_profile)
#cat("\\makebox[\\textwidth]{")
all_profiles = ''
for (profile in genomic_profiledf$genomic_profile) {
  # new_profile = sanitizeLatexS(profile)
  all_profiles = paste(all_profiles, profile, sep = " ")
  #cat(new_profile)
}
