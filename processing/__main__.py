...
def build_parser():
    ...
    parser_validate.add_argument("fileType", type=str, choices = PROCESS_FILES.keys(),
                        help='Filetypes that you are validating. Note, the filetypes with SP at the end are for special sponsored projects')
    parser_validate.add_argument("file", type=str, nargs="+",
    ...
    parser_validate.add_argument("--thread", type=int, required=False, default=1,
                        help='Number of threads used in validation symbols')
    parser_validate.add_argument("--offline", action='store_true',
    ...
    parser_validate.set_defaults(func=perform_validate)
    return(parser)
