...
def validateSymbol(x, bedDf, returnMappedDf=True):
    ...
    if sum(bedDf['Hugo_Symbol'] == gene) > 0:
        ...
        logger.info("%s will be remapped to %s" % (gene, mismatch['Hugo_Symbol'][0]))
        x['HUGO_SYMBOL'] = mismatch['Hugo_Symbol'][0]
    else:
...
class fusions(example_filetype_format.FileTypeFormat):
    ...
    def process_steps(self, filePath, **kwargs):
        ...
        temp.index=temp.HUGO_SYMBOL
        del temp['HUGO_SYMBOL']
        # fusion = fusion[~fusion['HUGO_SYMBOL'].isnull()]
        fusion['FUSION'] = fusion['FUSION'].fillna("")
        fusion, nonmapped = remapFusion(temp.to_dict()['ID'], fusion, "FUSION")
        fusion, nonmapped = remapFusion(temp.to_dict()['ID'], fusion, "COMMENTS")
    ...
    def validate_steps(self, filePathList, **kwargs):
        ...
        if fusionDF.get("COMMENTS") is None:
            fusionDF['COMMENTS'] = ""
        if not all([process_functions.checkColExist(fusionDF, i) for i in REQUIRED_HEADERS]):
            total_error += "Your fusion file must at least have these headers: %s.\n" % ",".join([i for i in REQUIRED_HEADERS if i not in fusionDF.columns.values])
        if process_functions.checkColExist(fusionDF, "HUGO_SYMBOL") and not noSymbolCheck:
            ...
            if fusionDF["HUGO_SYMBOL"].isnull().any():
                warning += "Any gene names can't be remapped will be removed.\n"
