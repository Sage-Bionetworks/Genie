...
# Get maf database
maf_synid = database_synid_mappingdf$Id[
  database_synid_mappingdf$Database == "vcf2maf"
]
mafdb = synTableQuery(sprintf('select Hugo_Symbol, Center, Chromosome, Tumor_Sample_Barcode, HGVSp_Short from %s', maf_synid),
                      includeRowIdAndRowVersion = F)
mafdbdf = mafdb$asDataFrame()
# Subset mafdf
mafdbdf = mafdbdf[mafdbdf$Tumor_Sample_Barcode %in% this_samples$SAMPLE_ID, ]
...
## Flagged Mutations
This is a count of how many flagged mutations a center has. Most of these variants are potential artifacts flagged by manual review of cBioPortal. These variants have been removed from the data. Please inform Sage Bionetworks:

* Suggestions for variants that should be part of this list
* If any variant shouldn't be part of this list

```{r blacklist}
blacklist_variants = paste(black_list_variantsdf$Hugo_Symbol,
                           black_list_variantsdf$HGVSp_Short)
subset_mut = mafdbdf[mafdbdf$Hugo_Symbol %in% black_list_variantsdf$Hugo_Symbol, ]
...
### Vital Status ('Missing' values)

These are the centers with 'missing' vital status
values: "Unknown", "Not Collected" for:

* YEAR_CONTACT

```{r yearcontact}
missing_vs = this_patient$YEAR_CONTACT %in% c("Unknown", 'Not Collected')

missing_subset = this_patient[missing_vs, ]
kable(table(missing_subset$YEAR_CONTACT, missing_subset$CENTER))
```
...
```{r intcontact}
missing_vs = this_patient$INT_CONTACT %in% c("Unknown", 'Not Collected')

missing_subset = this_patient[missing_vs, ]
kable(table(missing_subset$INT_CONTACT, missing_subset$CENTER))
```

* YEAR_DEATH

```{r yeardeath}
missing_vs = this_patient$YEAR_DEATH %in% c("Unknown", 'Not Collected')

missing_subset = this_patient[missing_vs, ]
kable(table(missing_subset$YEAR_DEATH, missing_subset$CENTER))
```

* INT_DOD

```{r intdod}
missing_vs = this_patient$INT_DOD %in% c("Unknown", 'Not Collected')

missing_subset = this_patient[missing_vs, ]
kable(table(missing_subset$INT_DOD, missing_subset$CENTER))
```

* DEAD

```{r dead}
missing_vs = this_patient$DEAD %in% c("Unknown", 'Not Collected')

missing_subset = this_patient[missing_vs, ]
kable(table(missing_subset$DEAD, missing_subset$CENTER))
```
...
duplicated_variants  = sum(duplicated_idx) > 0
```

`r if(duplicated_variants) {"---\n\n### Duplicated Variants\nThese duplicated variants will be removed if not corrected."}`

```{r duplicated, echo=F}
if (duplicated_variants) {
  kable(this_mut[duplicated_idx, c("Chromosome", "Start_Position",
                                   "Reference_Allele",
                                   "Tumor_Seq_Allele2",
...
`r if(nrow(non_somatic) > 0) {"---\n\n### Non-somatic Mutations\nThese mutations should be removed."}`

```{r nonsomatic, echo=F}
if (nrow(non_somatic) > 0) {
  kable(non_somatic[, c("Hugo_Symbol", "Chromosome", "Reference_Allele",
                        "Tumor_Seq_Allele1", "Tumor_Seq_Allele2",
                        "Tumor_Sample_Barcode")], row.names = F)

}
```


```{r invalid_variant_type, echo=F}
invalid_dnp <- this_mut[
  nchar(this_mut$Reference_Allele) == nchar(this_mut$Tumor_Seq_Allele2) &
    nchar(this_mut$Reference_Allele)==2 &
    this_mut$Variant_Type == "DNP",
]
if (nrow(invalid_dnp) > 0) {
  write.csv(invalid_dnp, "snv_as_dnp.csv", row.names=F, quote=F)
  # ent = synStore(File("snv_as_dnp.csv", parent=release_synid))
  dnp_ent = synStore(File("snv_as_dnp.csv", parent="syn4990358"))
  unlink("snv_as_dnp.csv")
}

invalid_onp <- this_mut[
  nchar(this_mut$Reference_Allele) == nchar(this_mut$Tumor_Seq_Allele2) &
    nchar(this_mut$Reference_Allele) == 3 &
    this_mut$Variant_Type == "ONP",]

if (nrow(invalid_onp) > 0) {
  write.csv(invalid_onp, "snv_as_onp.csv", row.names=F, quote=F)
  # ent = synStore(File("snv_as_onp.csv", parent=release_synid))
  onp_ent = synStore(File("snv_as_onp.csv", parent="syn4990358"))
  unlink("snv_as_onp.csv")
}

```

`r if(nrow(invalid_dnp) > 0) {paste0("---\n\n## SNV labelled as DNP\nThese are variants that have variant type DNP that should be SNV: ", "https://www.synapse.org/#!Synapse:", dnp_ent$properties$id)}`

`r if(nrow(invalid_onp) > 0) {paste0("---\n\n## SNV labelled as ONP\nThese are variants that have variant type ONP that should be SNV: ", "https://www.synapse.org/#!Synapse:", onp_ent$properties$id)}`
