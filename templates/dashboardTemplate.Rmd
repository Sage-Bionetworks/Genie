...
<!-- Only display if vital status exists -->

`r if(!is.null(this_samples$YEAR_CONTACT)) { "### Vital Status ('Missing' values)\nThese are the centers with 'missing' vital status values: 'Unknown', 'Not Collected' for:\n\n"}`

`r if(!is.null(this_samples$YEAR_CONTACT)) { "* YEAR_CONTACT\n"}`


```{r yearcontact}
if(!is.null(this_samples$YEAR_CONTACT)) {
...
`r if(!is.null(this_samples$YEAR_CONTACT)) { "* INT_CONTACT\n"}`
...
`r if(!is.null(this_samples$YEAR_CONTACT)) { "* YEAR_DEATH\n"}`
...
`r if(!is.null(this_samples$YEAR_CONTACT)) { "* INT_DOD\n"}`
...
`r if(!is.null(this_samples$YEAR_CONTACT)) { "* DEAD\n"}`
...
## Missing assay information
These are the SEQ_ASSAY_IDs that are present in the clinical file but not in the assay information file.  If your center's SEQ_ASSAY_ID is shown in this table, please add the panel to your assay information file.

```{r missingassay, echo=F}
missing_assays = this_assays[!this_assays %in% assay_infodf$SEQ_ASSAY_ID]
if (length(missing_assays) > 0) {
  knitr::kable(missing_assays, col.names=c("SEQ_ASSAY_ID"))
}
```
...
`r if(duplicated_variants) {"---\n\n### Duplicated Variants\nThese duplicated variants are caused by the annotation pipeline as no duplicated variants are allowed in the maf or vcfs."}`
...
`r if(nrow(non_somatic) > 0) {paste0("---\n\n### Non-somatic Mutations\nThese [mutations](",
                                     "https://www.synapse.org/#!Synapse:", non_somatic_ent$properties$id,
                                     "){target='_blank'} should be removed from the data.")}`
...
`r if(nrow(not_dnp) > 0) {paste0("---\n\n### SNV labelled as DNP\nThese are [variants](",
                                 "https://www.synapse.org/#!Synapse:", dnp_ent$properties$id,
                                 "){target='_blank'} that have variant type DNP that should be SNV")}`

`r if(nrow(not_onp) > 0) {paste0("---\n\n### SNV labelled as ONP\nThese are [variants](",
                                 "https://www.synapse.org/#!Synapse:", onp_ent$properties$id,
                                 "){target='_blank'} that have variant type ONP that should be SNV.")}`
