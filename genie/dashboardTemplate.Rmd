...
This is a count of how many blacklisted variants a center has. If your center shows up in this table, please investigate your variants. If you have suggestions for variants that should be part of this list, please inform Sage Bionetworks.
...
## Top 5 most frequently mutated genes per panel

To assist with discovering artifacts in a center's data, this will summarize the top 5 most frequently mutated genes per panel.  One of the easiest questions to answer is:

**Has the frequently mutated gene list changed significantly? Is TP53 still #1?**

### All panels

Top 5 most frequently mutated genes per panel for all panels.
...
mergeddf = merge.data.frame(this_mut[,c("Tumor_Sample_Barcode", "Hugo_Symbol")],
                            ...
                            by.y = "SAMPLE_ID")
mutated_genes = table(mergeddf$Hugo_Symbol, mergeddf$SEQ_ASSAY_ID)
...
final_matrix = cbind(transposed_matrix, table(mergeddf$SEQ_ASSAY_ID)[row.names(transposed_matrix)])
...
These are all the panels that cover TP53 that don't have TP53 as the top mutated genes.  Please review if data is correct.


```{r highlight_panels, echo=FALSE}

if (!is.null(this_bed)) {
  panels_covering_tp53 = unique(this_bed$SEQ_ASSAY_ID[this_bed$Hugo_Symbol == "TP53"])
...
Each panel's bed file is used to generate a gene panel file used in cBioPortal. All bed files undergo gene symbol harmonization during processing.
