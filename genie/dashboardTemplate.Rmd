...
`r if(check_seq_year) {"### Sequencing year\nThis is to show how many samples were sequenced per year. There are supposed to be at least 500 additional samples per consortium release per center."}`
...
### New samples submitted
This is the number of new samples submitted since the last public release. A gentle reminder for sites that it is GENIE policy to submit at least 500 samples between each public release.  Please contact AACR if you cannot do this.

```{r}
public_center_sample_count = table(public_sampledf$CENTER)
current_center_sample_count = table(this_samples$CENTER)
all_centers = union(names(public_center_sample_count),
                    names(current_center_sample_count))
center_differences = c()
for (center in all_centers) {
  center_current =  current_center_sample_count[center]
...
kable(center_differences, col.names = "Number of samples")
```
...
The GENIE retraction policy states that if a sample/patient is retracted within the first 3 months of a public release, it should be removed from the latest public release. This retraction will occur after the 3rd consortium release. If a sample is retracted after the 3 month period, it will not be released in the next public release.  This table below lists the number of samples explicitly or implicitly retracted by centers.

```{r}
kable(table(possible_retracted$CENTER, possible_retracted$RETRACTION))
```

 To confirm that we are retracting the right samples, we ask centers to confirm the retracted samples listed in this file: `r paste0("https://www.synapse.org/#!Synapse:", ent$properties$id)`. **Centers should only have samples in the linked file if someone has withdrew consent**. If you have samples/patients that should not be retracted, please upload a fix to your data or let Tom Yu from Sage know immediately.
