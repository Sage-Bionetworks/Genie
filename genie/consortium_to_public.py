...
def consortiumToPublic(syn, processingDate, genie_version,
    ...
    storeFile(syn, DATA_GENE_PANEL_PATH, PUBLIC_RELEASE_PREVIEW, genie_version,
    ...
    for entName, entId in consortiumRelease[2]:
        if "data_linear" in entName or "meta_" in entName:
            ...
            sampleCols.extend(publicRelease['fieldName'][
                publicRelease['level'] == "sample"].tolist())
            # clinicalDf is defined on line 127
            clinicalDf = \
                clinicalDf[clinicalDf['SAMPLE_ID'].isin(publicReleaseSamples)]
            ...
            storeFile(
                syn, FUSIONS_PATH, PUBLIC_RELEASE_PREVIEW,
                genie_version, name="data_fusions.txt")
        elif "CNA" in entName:
            ...
            with open(CNA_PATH, "w") as cnaFile:
                cnaFile.write(text)
            storeFile(
                syn, CNA_PATH, PUBLIC_RELEASE_PREVIEW,
                genie_version, name="data_CNA.txt")
            ...
            pass
            # This file was processed above because it had to be used
            # for generating caselists
            ...
            if len(checkRelease) == 3 and checkRelease[0] != "0":
                if int(checkRelease[1]) > 0:
                    if checkRelease[0] in ['1', '2']:
                        public_release_name = \
                            str(int(checkRelease[0])+1)+".0.0"
                    else:
                        public_release_name = \
                            str(int(checkRelease[0]))+".0-public"
