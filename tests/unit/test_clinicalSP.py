...
def test_perfect__processing():
	expected_clindf = pd.DataFrame(dict(PATIENT_ID=["GENIE-SAGE-ID1","GENIE-SAGE-ID2","GENIE-SAGE-ID3","GENIE-SAGE-ID4","GENIE-SAGE-ID5"],
...
def test_missingcols__validate():
	clindf = pd.DataFrame()
	error, warning = clinsp_class._validate(clindf)
	expected_errors = ("nonGENIE_data_clinical.txt: File must have SAMPLE_ID column.\n"
...
def test_errors__validate():
	...
	expected_errors = ("nonGENIE_data_clinical.txt: There can't be any blank values for SAMPLE_ID\n"
					  "nonGENIE_data_clinical.txt: There can't be any blank values for PATIENT_ID\n")
	expected_warnings = ("nonGENIE_data_clinical.txt: Please double check your SEQ_ASSAY_ID columns, there are empty rows.\n")
