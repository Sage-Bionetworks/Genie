...
def test_processing():
	...
	#TEST for remapping of columns
	patientDf = pd.DataFrame(dict(GENIE_PATIENT_ID=["ID1","ID2","ID3","ID4","ID5"],
								 NAACCR_SEX_CODE=[1,2,3,4,float('nan')],
								 NAACCR_RACE_CODE_PRIMARY=[1,2,3,4,float('nan')],
								 NAACCR_RACE_CODE_SECONDARY=[1,2,3,4,float('nan')],
								 NAACCR_RACE_CODE_TERTIARY=[1,2,3,4,float('nan')],
								 NAACCR_ETHNICITY_CODE=[1,2,3,4,float('nan')],
								 PATIENT_BIRTH_YEAR=[1990,1990,1990,1990,1990],
								 CENTER=["FOO","FOO","FOO","FOO","FOO"]))
	clinicalTemplate = pd.DataFrame(columns=patientCols)
	newPatientDf = clin._process(patientDf, clinicalTemplate)
	...
	#TEST for remapping of columns
	clinicalTemplate = pd.DataFrame(columns=sampleCols)
	sampleDf = pd.DataFrame(dict(GENIE_SAMPLE_ID=["ID1-1","ID2-1","ID3-1","ID4-1","ID5-1"],
								 PATIENT_ID=["ID1","ID2","ID3","ID4","ID5"],
								 AGE_SEQ_REPORT_DAYS=[100000,100000,100000,100000,100000],
								 ONCOTREE_CODE=['AMPCA','AMPCA','AMPCA','AMPCA','AMPCA'],
								 SAMPLE_TYPE=[1,2,3,4,4],
								 SEQ_ASSAY_ID=['SAGE-1','SAGE-1','SAGE-1','SAGE-1','SAGE-1'],
								 SEQ_DATE=['Jan-2012','Apr-2013','Jul-2014','Oct-2015','Release']))
...
def test_validation():
	...
	sampleDf = pd.DataFrame(dict(SAMPLE_ID=[float('nan'),"ID2-1","ID3-1","ID4-1","ID5-1"],
								 ...
								 ONCOTREE_CODE=['AMPCAD','AMPCA','AMPCA','AMPCA','AMPCA'],
	...
	expectedErrors = ("Sample: There can't be any blank values for SAMPLE_ID\n"
					  ...
					  "Patient: Please double check your SEX column.  This column must be these values 1, 2, 3, 4, or blank.\n"
					  "Patient: Please double check your ETHNICITY column.  This column must be these values 1, 2, 3, 4, or blank.\n")
	expectedWarnings = ("Sample: Please double check your SEQ_ASSAY_ID columns, there are empty rows.\n"
