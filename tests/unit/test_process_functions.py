...
def test_append__append_rows():
	...
	expecteddf = pd.DataFrame({'test':['test4'],
									  'foo':[4],
									  'baz':[3.2]})
	append_rows = genie.process_functions._append_rows(new_datadf, databasedf, 'UNIQUE_KEY')
	append_rows.fillna('',inplace=True)
	expecteddf.fillna('',inplace=True)
	assert append_rows.equals(expecteddf[append_rows.columns])

def test_norows__append_rows():
	append_rows = genie.process_functions._append_rows(databasedf, databasedf, 'UNIQUE_KEY')
...
def test_update__update_rows():
	...
	expecteddf = pd.DataFrame({"test":['test','test2'],
						  ...
						  'ROW_ID':['1','2'],
						  'ROW_VERSION':['3','3']})
	update_rows = genie.process_functions._update_rows(new_datadf, databasedf, 'UNIQUE_KEY')
...
def test_norows__update_rows():
	update_rows = genie.process_functions._update_rows(databasedf, databasedf, 'UNIQUE_KEY')
	assert update_rows.empty
...
def test_delete__delete_rows():
	...
	expecteddf = pd.DataFrame({0:['2','3'],
						  	   1:['3','5']})
	delete_rows = genie.process_functions._delete_rows(new_datadf, databasedf, 'UNIQUE_KEY')
	assert delete_rows.equals(expecteddf)

def test_norows__delete_rows():
	delete_rows = genie.process_functions._delete_rows(databasedf, databasedf, 'UNIQUE_KEY')
