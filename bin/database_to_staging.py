...
flowchart TD
    ...
    S4 --> S5["Run MAFinBED, mutationInCis, seq date, gene panel filters"]
    S5 --> SD1["Variant Filters"]
    SD1 --> SD2["Germline filter: If any variant has a max gnomAD AF from any subpopulation that is over 0.0005, the variant will be filtered out."]
    SD1 --> SD3["MAF in BED: Any variants in the mutation file that isn't described by the bed file will be filtered out"]
    SD2 & SD3 --> SD5["List of variants to remove"]
    S5 --> SE1["Sample Filters"]
    SE1 --> SE2["SEQ_DATE: Samples from every major release and its associated consortium releases are vetted by its SEQ_DATE. For instance, samples in the 5.0 consortium and public releases will only contain samples that were sequenced prior to Jan-2018 (not including Jan-2018)"]
    SE1 --> SE3["No Bed file: Samples that have a SEQ_ASSAY_ID that don't have a bed file associated with them will be removed"]
    SE1 --> SE4["Onctoree: Samples with deprecated codes (as defined by the oncotree source version used in the release) will be removed."]
    SE1 --> SE5["Mutation In Cis: Samples that have variants that could be merged together will be filtered out. This can be turned off."]
